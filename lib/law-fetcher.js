...
async function updateLaws() {
  ...
  process.stderr.write(chalk.cyan('Updating urban planning laws...\n'));
  const bar = new cliProgress.SingleBar({
    format: 'Progress |' + chalk.cyan('{bar}') + '| {percentage}% | {value}/{total} Laws',
    barCompleteChar: '\u2588',
    barIncompleteChar: '\u2591',
    hideCursor: true
  }, cliProgress.Presets.shades_classic);
  
  // Ensure progress bar uses stderr
  bar.terminal = process.stderr;
